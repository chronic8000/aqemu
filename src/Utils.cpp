...
void AQEMU_Startup_Log( const char *stage )
{
	...
	if( Save_Messages_To_Log )
		AQSave_To_Log( QStringLiteral( "Startup" ), QStringLiteral( "AQEMU" ),
			       QString::fromUtf8( stage ) );
}

void AQEMU_Startup_Log( const QByteArray &stage )
{
	AQEMU_Startup_Log( stage.constData() );
}

void AQEMU_Startup_Log( const QString &stage )
{
	const QByteArray utf8 = stage.toUtf8();
	AQEMU_Startup_Log( utf8.constData() );
}
...
bool AQ_Apply_QEMU_Dir_As_Default_Emulator( const QString &dir_in, const QString &display_name )
{
	...
	if( name.isEmpty() )
	{
		// Prefer real version label from first available binary
		for( QMap<QString, QString>::const_iterator it = qemu_list.constBegin(); it != qemu_list.constEnd(); ++it )
		{
			if( ! it.value().isEmpty() && QFile::exists( it.value() ) )
			{
				name = System_Info::Get_Emulator_Version_Label( it.value() );
				break;
			}
		}
